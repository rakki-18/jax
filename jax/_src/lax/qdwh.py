...
def _use_cholesky(u, params):
  ...
  # Adapted from `_cholesky` in jax/_src/scipy/linalg.py.
  def cholesky(a, lower=False):
    a = np_linalg._promote_arg_dtypes(jnp.asarray(a))
    l = lax_linalg.cholesky(jnp.conj(jnp.swapaxes(a, -1, -2)),
                            symmetrize_input=False)
    return jnp.conj(jnp.swapaxes(l, -1, -2))

  y = cholesky(x)

  # Adapted from `_solve_triangular` in jax/_src/scipy/linalg.py.
  def solve_triangular(a, b, lower=False):
    a, b = np_linalg._promote_arg_dtypes(jnp.asarray(a), jnp.asarray(b))
    return lax_linalg.triangular_solve(a, b, left_side=True, lower=lower,
                                       transpose_a=False,
                                       conjugate_a=False,
                                       unit_diagonal=False)

  z = solve_triangular(y.T, u.T, lower=True).conj()
  z = (solve_triangular(y, z)).T.conj()
